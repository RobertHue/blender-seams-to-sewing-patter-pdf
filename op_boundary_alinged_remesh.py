...
class BoundaryAlignedRemesher:
    ...
    def remesh(
        self, edge_length=0.05, iterations=30, quads=True, reproject=True
    ):
...
class Remesher(bpy.types.Operator):
    ...
    edge_length: bpy.props.FloatProperty(name="Edge Length", min=0, default=0.1)

    iterations: bpy.props.IntProperty(name="Iterations", min=1, default=30)

    quads: bpy.props.BoolProperty(name="Quads", default=False)
